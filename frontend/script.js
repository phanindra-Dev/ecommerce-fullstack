...
async function fetchProducts() {
    try {
        const res = await fetch(getApiUrl('PRODUCTS_ALL'), {
            method: 'GET',
            credentials: 'include'
        });
...
async function fetchCart() {
    ...
    try {
        const res = await fetch(`${getBaseApiUrl()}${API_CONFIG.ENDPOINTS.CART_VIEW}/${currentUser.userId}`, {
            method: 'GET',
            credentials: 'include'
        });
...
async function addToCart(productId) {
    ...
    try {
        ...
        if (existingItem) {
            ...
            const updateRes = await fetch(`${getBaseApiUrl()}${API_CONFIG.ENDPOINTS.CART_UPDATE}/${existingItem.cartItemId}?quantity=${updatedQuantity}`, {
            ...
            const addRes = await fetch(`${getBaseApiUrl()}${API_CONFIG.ENDPOINTS.CART_ADD}?userId=${currentUser.userId}&productId=${productId}&quantity=1`, {
...
async function removeFromCart(cartItemId) {
    try {
        const res = await fetch(`${getBaseApiUrl()}${API_CONFIG.ENDPOINTS.CART_REMOVE}/${cartItemId}`, {
            method: 'DELETE',
            credentials: 'include'
        });
...
async function updateCartQuantity(cartItemId, quantity) {
    try {
        const res = await fetch(`${getBaseApiUrl()}${API_CONFIG.ENDPOINTS.CART_UPDATE}/${cartItemId}?quantity=${quantity}`, {
            method: 'PUT',
            credentials: 'include'
        });
        const updatedCart = await res.json();
...
async function fetchTotalPrice() {
    ...
    try {
        const res = await fetch(`${getBaseApiUrl()}${API_CONFIG.ENDPOINTS.CART_TOTAL}/${currentUser.userId}`, {
            method: 'GET',
            credentials: 'include'
        });
...
document.getElementById('buyBtn').onclick = async () => {
    ...
    try {
        ...
        const response = await fetch(getApiUrl('ORDERS_PLACE'), {
            method: 'POST',
...
async function checkSession() {
    ...
    if (storedUser) {
        ...
        try {
            const res = await fetch(getApiUrl('ME'), { method: "GET", credentials: "include" });
            if (!res.ok) return;
